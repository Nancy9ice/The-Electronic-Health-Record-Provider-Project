...
SELECT CONVERT(VARCHAR(32), HASHBYTES('MD5', CONVERT(VARBINARY(MAX), COALESCE(hospital, '_dbt_utils_surrogate_key_null_'))), 2) AS hospital_key,
...
GROUP BY CONVERT(VARCHAR(32), HASHBYTES('MD5', CONVERT(VARBINARY(MAX), COALESCE(hospital, '_dbt_utils_surrogate_key_null_'))), 2),
